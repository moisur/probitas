...
const AboutPage: React.FC = () => {
  ...
  const values = [
    { name: "Empathie", text: "Comprendre les émotions et besoins de nos clients pour les aider à surmonter leurs difficultés." },
    { name: "Adaptabilité", text: "Vigie constante sur les évolutions rapides de la compliance et des nouveaux outils." },
    { name: "Nuance", text: "Admettre la complexité des situations de terrain pour éviter les généralisations hâtives." },
    { name: "Créativité", text: "Solutions innovantes et pensée créative pour une amélioration constante." },
    { name: "Communication", text: "Expression claire, écoute active et transmission d'informations sans ambiguïté." }
  ];
          ...
          <div className="lg:w-2/3 grid grid-cols-1 md:grid-cols-2 gap-12">
            {values.map((v, i) => (
              <div key={i} className="relative pl-8">
                <div className="absolute left-0 top-2 w-4 h-[1px] bg-[#BF9B8E]" />
                <h4 className="text-white font-cinzel font-bold text-lg mb-4 uppercase">{v.name}</h4>
                <p className="text-white/40 font-light text-sm">{v.text}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-[#BF9B8E] p-12 md:p-20 text-center relative overflow-hidden">
          <div className="relative z-10">
            ...
            <form onSubmit={handleSubscribe} className="flex flex-col md:flex-row gap-4 justify-center max-w-md mx-auto relative cursor-pointer">
              ...
              <button
                type="submit"
                className={`flex items-center gap-3 font-bold uppercase tracking-widest px-8 py-4 transition-all duration-500 ${status === 'success' ? 'bg-[#283618] text-white' : 'bg-[#0C2E59] text-white hover:bg-[#1a4a8a]'}`}
              >
